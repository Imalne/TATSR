...
from src.model.non_local.IPT import PatchEmbed, EncoderLayer, DePatchEmbed
from src.model.non_local.swin_transformer import BasicLayer
...
class MultiHeadPosAttn(Module):
    ...
    def __init__(self, in_dim, norm_layer, n_head=4):
        ...
        self.query_conv = nn.ModuleList(
            [Conv2d(in_channels=in_dim, out_channels=in_dim // n_head, kernel_size=1) for _ in range(n_head)])
        ...
        self.feed_forward = nn.Sequential(Conv2d(in_channels=in_dim, out_channels=self.dim_feedforward, kernel_size=1),
                                          nn.PReLU(),
                                          Conv2d(in_channels=self.dim_feedforward, out_channels=in_dim,
                                                 kernel_size=1), )
    ...
    def forward(self, x):
        m_batchsize, C, height, width = x.size()
        proj_querys = [self.query_conv[i](x).view(m_batchsize, -1, width * height).permute(0, 2, 1) for i in
                       range(self.n_head)]
...
class ViTransformerEncoder(nn.Module):
    def __init__(self, channels, input_size, patch_size=1, num_heads=8, ffn_ratio=4, num_layer=3, drop_rate=0.1,
                 qkv_bias=False, qk_scale=None, attn_drop_rate=0., norm_layer=nn.LayerNorm):
        super(ViTransformerEncoder, self).__init__()
        self.patch_embedding = PatchEmbed(patch_size=patch_size, in_channels=channels)
        self.embed_dim = self.patch_embedding.dim
        if self.embed_dim % num_heads != 0:
            raise RuntimeError("Embedding dim must be devided by numbers of heads")

        self.pos_embed = nn.Parameter(
            torch.zeros(1, (input_size[0] // patch_size) * (input_size[1] // patch_size), self.embed_dim))
        self.encoder = nn.ModuleList([
            EncoderLayer(
                dim=self.embed_dim, num_heads=num_heads, ffn_ratio=ffn_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                drop=drop_rate, attn_drop=attn_drop_rate, norm_layer=norm_layer)
            for _ in range(num_layer)])
        self.de_patch_embedding = DePatchEmbed(patch_size=patch_size, in_channels=channels)
...
class SwinTransformerEncoder(nn.Module):
    def __init__(self, channels, input_size, patch_size=1, num_heads=8, ffn_ratio=4, num_layer=3, drop_rate=0.1,
        ...
        # self.pos_embed = nn.Parameter(torch.zeros(1, (input_size[0] // patch_size) * (input_size[1] // patch_size),
        # self.embed_dim))
        self.encoder = BasicLayer(self.embed_dim, input_resolution=input_size, depth=num_layer, num_heads=num_heads,
                                  window_size=8, mlp_ratio=ffn_ratio, qkv_bias=qkv_bias, qk_scale=qk_scale,
                                  drop=drop_rate, attn_drop=attn_drop_rate, downsample=None, norm_layer=norm_layer)
        self.de_patch_embedding = DePatchEmbed(patch_size=patch_size, in_channels=channels)
